...
export default async function DashboardPaiPage() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 bg-[#0B0F19]">
      {/* Contêiner Pai - Simulador de Smartphone Topo de Linha */}
      <div className="max-w-md w-full min-h-[85vh] md:min-h-[800px] bg-[#121826] rounded-[32px] border border-zinc-800 shadow-2xl flex flex-col relative overflow-hidden">
        
        {/* Header Minimalista */}
        <header className="p-6 flex justify-between items-center bg-[#161F30]/40 border-b border-zinc-800/50">
          <div>
            <h1 className="font-bold text-xl text-white font-title tracking-tight">
              Olá, {nome}
            </h1>
        ...
        {/* Conteúdo Central */}
        {aluno ? (
          <main className="flex-1 w-full flex flex-col p-6 overflow-y-auto scrollbar-hide relative z-10 space-y-6">
            {/* Card de Status do Trajeto */}
            <section className="bg-[#1A2333]/60 border border-zinc-800 p-6 rounded-[24px] shadow-xl relative overflow-hidden">
          ...
          <main className="flex-1 flex flex-col items-center justify-center px-6 text-center animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10">
            {/* Card de Vidro Escuro (Estado Vazio Escolar Pro) */}
            <div className="bg-[#1A2333]/60 border border-zinc-800 p-8 rounded-[24px] w-full shadow-xl">
              ...
              <p className="text-zinc-400 text-sm mt-2 leading-relaxed">
                Encontre o Tio da Van ideal que atende a escola do seu filho e a sua região para começar o monitoramento.
              </p>
              
              {/* O BOTÃO BOMBA DE CTA (O Grande Gatilho) */}
              <Link 
                href="/dashboard/pai/buscar"
                className="block w-full mt-6 bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3.5 rounded-xl transition-all text-sm tracking-wide shadow-lg shadow-blue-900/20 active:scale-[0.98]"
              >
